...
class ScanWindow(QDialog):
    def __init__(self, db_manager, parent=None, sala_id=None):
        ...
        self.move(
            int((screen.width() - width) / 2), int((screen.height() - height) / 2)
        )

        layout = QVBoxLayout()
        layout.addStretch(1)

        label = QLabel("Escaneie o número do livro:")
        label.setFont(QFont("Arial", 16))
        label.setAlignment(Qt.AlignCenter)
        layout.addWidget(label)

        sala_nome = self.get_sala_nome()
        self.sala_label = QLabel(f"Estante: {sala_nome}")
    ...
    def get_sala_nome(self):
        ...
        if self.sala_id:
            self.db_manager.cursor.execute(
                "SELECT sala FROM salas WHERE id = ?", (self.sala_id,)
            )
            result = self.db_manager.cursor.fetchone()
            return result[0] if result else "Desconhecida"
        return "Nenhuma estante selecionada"
    ...
    def process_scan(self):
        ...
        if not self.sala_id:
            self._set_feedback("Nenhuma estante selecionada.", "gray")
            self.input.clear()
            return
        ...
        if status is None:
            ...
            self._set_feedback(
                f"Livro {numero} não cadastrado no sistema — registrado.",
                "rgb(180, 0, 0)",
            )
            ...
            self._set_feedback(
                f"Livro {numero} já havia sido marcado como encontrado.",
                "rgb(180, 100, 0)",
            )
            ...
            sala_nome = self.sala_label.text().replace("Estante: ", "")
            self._set_feedback(
                f"Livro {numero} encontrado na estante {sala_nome}.",
                "rgb(0, 130, 0)",
            )
    ...
    def closeEvent(self, event):
        ...
        # Calcular itens ainda não encontrados na estante
        pendentes = 0
        if self.sala_id:
            try:
                self.db_manager.cursor.execute(
        ...
        total_sessao = (
            self._encontrados_sessao
            + self._nao_cadastrados_sessao
            + self._duplicados_sessao
        )
        if total_sessao > 0:
            QMessageBox.information(
                ...
                f"Livros ainda pendentes na estante: {pendentes}",
            )

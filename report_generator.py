...
def _generate_report(db_manager) -> str:
    ...
    headers_geral = ["Estante Atual"] + headers_sala
    ...
    _write_csv(
        geral_dir / "encontrados.csv",
        headers_geral,
        [[sala_nome] + pat_row(pat, "Lido") for sala_nome, pat in geral_encontrados],
    )
    _write_csv(
        geral_dir / "nao_encontrados.csv",
        headers_geral,
        [
            [sala_nome] + pat_row(pat, "Não Lido")
            for sala_nome, pat in geral_nao_encontrados
        ],
    )
    _write_csv(
        geral_dir / "divergente.csv",
        headers_geral,
        [
    ...
    for sala_id, sala_info in salas.items():
        ...
        safe_nome = "".join(
            c if c.isalnum() or c in ("_", "-") else "_" for c in sala_nome
        )

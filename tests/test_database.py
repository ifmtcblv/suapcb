...
class TestLimparTombo:
    def test_no_prefix(self):
        assert limpar_tombo("123456") == "123456"
        assert limpar_tombo("12") == "000012"
        assert limpar_tombo("000001") == "000001"

    def test_with_matching_prefix(self):
        assert limpar_tombo("18000001", prefixo_tombo="18") == "000001"
        assert limpar_tombo("18000123", prefixo_tombo="18") == "000123"
        assert limpar_tombo("1800000001", prefixo_tombo="18") == "000001"
        assert limpar_tombo("18000001/A", prefixo_tombo="18") == "000001"

    def test_with_non_matching_prefix(self):
        assert limpar_tombo("18000001", prefixo_tombo="12") == "18000001"
        assert limpar_tombo("000001", prefixo_tombo="18") == "000001"
...
class TestDatabaseManager:
    ...
    def test_mark_patrimonio_encontrado(self, tmp_path):
        db = DatabaseManager(db_path=tmp_path / "test.db")
        db.cursor.execute(
            "INSERT INTO salas (id, sala, codigo) VALUES (1, 'SALA A', 'abc')"
        )
        db.cursor.execute(
...
class TestLoadDataFromFile:
    ...
    def test_invalid_csv_does_not_delete_existing_data(self, tmp_path):
        db = DatabaseManager(db_path=tmp_path / "test.db")
        db.cursor.execute(
            "INSERT INTO salas (id, sala, codigo) VALUES (1, 'EXISTENTE', 'abc')"
        )
        db.conn.commit()
    ...
    def test_gnuteca_format_detection_and_load(self, tmp_path):
        headers = (
            '"Nº de controle";"Número do tombo";"Estado";"Tipo de material";"Nº da obra";'
            '"Patrimônio";"Data de entrada";"Nº de chamada";"Título";"Autor";"Volume";'
            '"Exemplar";"Edição";"Lugar pub.";"Editora";"Data pub."\n'
        )
        row = (
            '"559";"18000001";"Disponível";"Livro";"555";"";"09/12/2013";'
            '"636.52 C846a";"Alimentação de aves";"Cotta, Tadeu";"";"Ex. 01";"";'
            '"Viçosa- MG";"Aprenda Fácil";"2003"\n'
        )
        csv_path = tmp_path / "gnuteca.csv"
        csv_path.write_text(headers + row, encoding="utf-8")

        # Teste de detecção
        fmt, delimiter = _detect_csv_format(str(csv_path))
        assert fmt == "GNUTECA"
        assert delimiter == ";"
        ...
        salas = db.get_all_salas()
        assert len(salas) == 1
        assert salas[0][1] == "BIBLIOTECA"

        rows = db.get_patrimonios_by_sala(salas[0][0])
        assert len(rows) == 1
        assert rows[0][0] == "18000001"
        assert rows[0][1] == "Disponível"
        assert rows[0][3] == "Alimentação de aves - Cotta, Tadeu (Ex. 01)"
        assert rows[0][4] == "Livro"
        db.close()

    def test_loads_csv_with_prefix_suap(self, tmp_path):
        row = (
            "1,12000001,Ativo,ED01,Mesa,,SETOR A,RESP A,BLV,500.00,400.00,NF001,"
            "SN001,2020-01-01,2020-01-02,FORN A,SALA 101,Bom,\n"
        )
        csv_path = tmp_path / "data_prefix.csv"
        csv_path.write_text(_make_csv([row]), encoding="utf-8")

        db = DatabaseManager(db_path=tmp_path / "test.db")
        load_data_from_file(db.cursor, db.conn, str(csv_path), prefixo_tombo="12")
    ...
    def test_loads_csv_prefix_not_matching_suap(self, tmp_path):
        row = (
            "1,13000002,Ativo,ED01,Mesa,,SETOR A,RESP A,BLV,500.00,400.00,NF001,"
            "SN001,2020-01-01,2020-01-02,FORN A,SALA 101,Bom,\n"
        )
        csv_path = tmp_path / "data_no_match.csv"
        csv_path.write_text(_make_csv([row]), encoding="utf-8")
        ...
        salas = db.get_all_salas()
        rows = db.get_patrimonios_by_sala(salas[0][0])
        assert rows[0][0] == "13000002"
        db.close()

    def test_loads_csv_zfill_suap(self, tmp_path):
        row = (
            "1,123,Ativo,ED01,Mesa,,SETOR A,RESP A,BLV,500.00,400.00,NF001,"
            "SN001,2020-01-01,2020-01-02,FORN A,SALA 101,Bom,\n"
        )
        csv_path = tmp_path / "data_zfill.csv"
        csv_path.write_text(_make_csv([row]), encoding="utf-8")
    ...
    def test_loads_csv_with_prefix_gnuteca(self, tmp_path):
        headers = (
            '"Nº de controle";"Número do tombo";"Estado";"Tipo de material";"Nº da obra";'
            '"Patrimônio";"Data de entrada";"Nº de chamada";"Título";"Autor";"Volume";'
            '"Exemplar";"Edição";"Lugar pub.";"Editora";"Data pub."\n'
        ...
        csv_path = tmp_path / "gnuteca_prefix.csv"
        csv_path.write_text(headers + row, encoding="utf-8")

        db = DatabaseManager(db_path=tmp_path / "test.db")
        load_data_from_file(db.cursor, db.conn, str(csv_path), prefixo_tombo="18")

...
class LoadWorker(QThread):
    """Carrega um CSV exportado do SUAP ou Gnuteca em thread separada, evitando congelamento da UI."""

    finished = pyqtSignal(int, int)  # (n_patrimonios, n_salas)
    error = pyqtSignal(str)

    def __init__(self, db_manager, file_path, prefixo_tombo=None):
        super().__init__()
        self.db_manager = db_manager
        self.file_path = file_path
        self.prefixo_tombo = prefixo_tombo
    ...
    def run(self):
        ...
        try:
            ...
            sala_data, patrimonios_data = _parse_csv(self.file_path, prefixo_tombo=self.prefixo_tombo)
...
class MainWindow(QMainWindow):
    def __init__(self, db_manager):
        super().__init__()
        self.setWindowTitle("SUAP-CB - Conferência de Livros")
        self.db_manager = db_manager
        ...
        # Título
        label = QLabel("Bem-vindo ao SUAP-CB!", self)
        ...
        scan_button = QPushButton("Escanear Livros")
        ...
        self.patrimonio_table.horizontalHeader().setSectionResizeMode(
            QHeaderView.Stretch
        )
        ...
        self.total_label = QLabel("Total de Livros: 0")
        self.total_label.setFont(QFont("Arial", 12))
        stats_layout.addWidget(self.total_label)

        self.encontrados_label = QLabel("Livros Encontrados: 0")
    ...
    def _build_legenda(self):
        ...
        itens = [
            ("#90EE90", "Encontrado nesta estante"),
            ("#FFFF00", "Encontrado em outra estante (divergente)"),
        ]
    ...
    def update_patrimonios_table(self):
        ...
        if not selected_items:
            ...
            self.total_label.setText("Total de Livros: 0")
            self.encontrados_label.setText("Livros Encontrados: 0")
            return
        ...
        self.total_label.setText(f"Total de Livros: {total_patrimonios}")
        self.encontrados_label.setText(f"Livros Encontrados: {encontrados}")
    ...
    def _executar_busca(self, numero):
        ...
        self.total_label.setText(f"Total de Livros: {total}")
        self.encontrados_label.setText(f"Livros Encontrados: {encontrados}")
    ...
    def open_scan_window(self):
        ...
        if not selected_items:
            QMessageBox.warning(
                self, "Erro", "Por favor, selecione uma sala antes de escanear."
            )
            return
    ...
    def carregar_arquivo(self):
        """Abre um seletor de arquivo para carregar um CSV exportado do SUAP ou Gnuteca."""
        caminho, _ = QFileDialog.getOpenFileName(
            self,
            "Selecionar arquivo CSV do SUAP ou Gnuteca",
            "",
            "Arquivos CSV (*.csv *.CSV);;Todos os arquivos (*)",
        )
        if not caminho:
            return
        ...
        confirmacao = QMessageBox.warning(
            ...
            "(livros escaneados e marcações de encontrado).\n\n"
            "Deseja continuar?",
        ...
        while True:
            text, ok = QInputDialog.getText(
                self,
                "Prefixo do Tombo",
                "Digite o número de 2 dígitos do tombo (deixe em branco se não houver):"
            )
            if not ok:
                return
            ...
            if len(text) == 2 and text.isdigit():
                ...
                QMessageBox.warning(
                    self,
                    "Prefixo Inválido",
                    "Por favor, insira exatamente um número de 2 dígitos ou deixe em branco."
                )

        self.load_button.setEnabled(False)
        self.load_button.setText("Carregando...")

        self._load_worker = LoadWorker(self.db_manager, caminho, prefixo_tombo=prefixo_tombo)
